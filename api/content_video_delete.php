...
if ($url === '') {
    jsonResponse(['ok' => false, 'error' => 'Нужен url видео'], 400);
}

$prefix = '/uploads/content_videos/';
...
try {
    ...
    if ($linksCount > 0) {
        jsonResponse(['ok' => false, 'error' => 'Видео используется в других контентах и не может быть удалено'], 409);
    }
    ...
    jsonResponse(['ok' => false, 'error' => 'Не удалось удалить видео'], 500);
}
