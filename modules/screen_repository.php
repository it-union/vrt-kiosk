...
function screenResolveIdentity(int $screenId): array
{
...
function screenEnsureExists(PDO $pdo, int $screenId): int
{
    static $ready = [];
    $identity = screenResolveIdentity($screenId);
    $storageId = (int)$identity['storage_id'];
    $deviceKey = (string)$identity['device_key'];

    if (isset($ready[$deviceKey])) {
        return $ready[$deviceKey];
    }

    $stmt = $pdo->prepare('SELECT id FROM screens WHERE device_key = :device_key LIMIT 1');
    $stmt->execute([':device_key' => $deviceKey]);
    $existingId = $stmt->fetchColumn();
    if ($existingId !== false) {
        $ready[$deviceKey] = (int)$existingId;
...
function resolveScreenStorageId(PDO $pdo, int $screenId): int
{
    return screenEnsureExists($pdo, $screenId);
}
